...
export function getConfig() {
  ...
  return {
    BACKUP_DIR: process.env.BACKUP_DIR as string,
...
export function getLatestBackupFilePath() {
  const { BACKUP_DIR } = getConfig();
  return path.join(BACKUP_DIR, "latest.sql");
}

export function getNextBackupFilePath() {
  const { DB_NAME, BACKUP_DIR } = getConfig();
  const srcDir = BACKUP_DIR;
